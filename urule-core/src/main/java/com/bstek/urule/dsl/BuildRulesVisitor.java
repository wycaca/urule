...
public class BuildRulesVisitor extends RuleParserBaseVisitor<Object> {
	private final Map<ParseTree, Junction> map = new HashMap<ParseTree, Junction>();
	private final Collection<ContextBuilder> builders;
	private final NamedConditionBuilder namedConditionBuilder = new NamedConditionBuilder();
	...
	public Criterion visitMultiConditions(MultiConditionsContext ctx) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (parseTree instanceof JoinContext) {
				...
				boolean isMulti = parseTree instanceof MultiConditionsContext;
