...
public class ElCalculator {
    private static final int[] PRIORITY = new int[]{0, 3, 2, 1, -1, 1, 0, 2};// 运用运算符ASCII码-40做索引的运算符优先级
    ...
    public boolean compare(char cur, char peek) {// 如果是peek优先级高于cur，返回true，默认都是peek优先级要低
        if (cur == '%') cur = '*';
        if (peek == '%') peek = '*';
        boolean result = PRIORITY[(peek) - 40] >= PRIORITY[(cur) - 40];
        return result;
    }
    ...
    private Object calculate(String firstValue, String secondValue, char currentOp) {
        ...
        try {
            ...
            String a = (firstValue == null ? "null" : firstValue);
            String b = (secondValue == null ? "null" : secondValue);
            if (currentOp == '+') {
                result = a + b;
